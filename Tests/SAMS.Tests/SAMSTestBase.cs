...
namespace SAMS.Tests
{
    public abstract class SAMSTestBase : AbpIntegratedTestBase<SAMSTestModule>
    {
        ...
        protected void UsingDbContext(Action<SAMSDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<SAMSDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<SAMSDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<SAMSDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<SAMSDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SAMSDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<SAMSDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SAMSDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<SAMSDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<SAMSDbContext, Task<T>> func)
        {
            T result;
