...
using SAMS.Users;
using SAMS.Web.Controllers.Results;
using SAMS.Web.Models.Account;
...
{
    public class AccountController : SAMSControllerBase
    {
        ...
        public ActionResult Login(string returnUrl = "")
        {
            ...
            else
            {
                ...
                if (returnUrl.Contains("ticket=")  &&!AuthenticationManager.User.Identity.IsAuthenticated)
                {
                    //云之家ticket登录
                    string url = returnUrl.Split("?")[1];
                    var ticket = "";
                    string[] q = url.Split("&");
                    ...
                    if (!string.IsNullOrWhiteSpace(ticket))
                    {
                        ...
                        {
                            AsyncHelper.RunSync(() => SignInAsync(loginResult.User, loginResult.Identity));
                            Redirect(returnUrl);
                        }
                    }
